        ...
        <div id="users-box" class="col-lg-12 layout-spacing">
            <div class="statbox widget box box-shadow">
                ...
                <div class="widget-content widget-content-area">
                    @can(Acl::PERMISSION_ROLE_ADD_HR)
        ...
        const c1 = $('#datatable').DataTable({
            ...
            "columns": [{
                ...
                },
                {
                    ...
                    "render": function(data, type, full, meta) {
                        let col = '';
                        data.slice(0, 15).forEach(permission => col += `<span class="badge badge-info me-2 mb-2">${permission.name}</span>`);
                        return col;
                    },
                },
                {
                    ...
                    "render": function(data, type, full) {
                        let urlEdit = `{{ route('admin.role.edit', ':id') }}`.replace(':id', data);
                        let urlDestroy = `{{ route('admin.role.destroy', ':id') }}`.replace(':id',
                            data);

                        return `
                            <div class="action-btns">
                                <x-table.actions.edit-action
                                    :permission="Acl::PERMISSION_ROLE_EDIT_HR"
                                    :url="'${urlEdit}'"
                                />
                                <x-table.actions.delete-action
                                    :permission="Acl::PERMISSION_ROLE_DELETE_HR"
                                    :url="'${urlDestroy}'"
                                />
                            </div>
                        `;
